...
class CSV(Resource):
    ...
    def post(self):
        try:
            ...
            #     f'./{BaseConfig.GO_REST_EXCEL_FILENAME}', BaseConfig.GO_REST_BUCKETNAME, BaseConfig.GO_REST_EXCEL_FILENAME)
            # logging.info(f'Finish to upload Csv file with all GoRest data to s3')
            # os.remove(f'./{BaseConfig.GO_REST_EXCEL_FILENAME}')
            # if (isUploaded == False):
    ...
    def post(self):
        try:
            ...
            def on_close():
                ...
                entities = createFakeEntities(10)
                dataCreated = asyncio.run(createGoRestEntities(entities))
                json_object = json.dumps(dataCreated, indent=4)
                with open(f'{BaseConfig.STORE_NAME}-{lastRunUUID}.json', "w") as outfile:
                    outfile.write(json_object)
                simple_cache.save_key(BaseConfig.STORE_NAME, lastRunUUID, dataCreated, 99999999999999999)
            return response
        except Exception as e:
...
class GetEntities(Resource):
    ...
    def get(self,scan_id):
        try:
            ...
            scanID = scan_id
            isExist = os.path.exists(f'./{BaseConfig.STORE_NAME}-{scanID}.json')
            if(isExist == False):
                return {"data": None, "statusCode": HttpStatusCode.INTERNAL_ERROR, "message":'scan is not finished'},  HttpStatusCode.INTERNAL_ERROR
            data = simple_cache.load_key(BaseConfig.STORE_NAME, f'{BaseConfig.STORE_NAME}-{scanID}')
            if(data):
                return {"data": data, "statusCode": HttpStatusCode.OK, "message": None}, HttpStatusCode.OK
            with open(f'{BaseConfig.STORE_NAME}-{scanID}.json', 'r') as openfile:
